...
module.exports = {
  ...
  updateVersion: (cwd, version, callback) => {
	...
	const versionedFile = `${cwd}/docker-compose.yml`
	const yamlContents = yaml.parse(fs.readFileSync(versionedFile, 'utf8'));
	yamlContents.services.runner.image = yamlContents.services.runner.image.replace(regex, `$1:v${version}`);
	fs.writeFileSync(versionedFile, yaml.stringify(yamlContents));
  },
